...
with tab1:
    ...
    if st.button("Собрать", key="btn_url"):
        ...
        try:
            ...
            st.session_state.df_url = df
        except Exception as e:
            st.error("Ошибка при запросе GA4")
            st.code(str(e))
            st.stop()

    df_url = st.session_state.df_url
    if df_url is not None:
        show = df_url.reindex(
            ...
            columns={
                ...
                "avgSessionDuration_sec": "Средняя длительность сеанса (сек)",
            }
        )
...
with tab2:
    ...
    if df_top is not None:
        ...
        st.download_button(
            "Скачать CSV",
            show_top.to_csv(index=False).encode("utf-8-sig"),
...
with tab3:
    ...
    if st.button("Обновить итоги", key="btn_totals"):
        ...
        try:
            with st.spinner("Обновляю итоги…"):
